...
Arguments:
    --backend   : 'sd', 'sd15', or 'edm' (required)
    --scorer    : 'brightness', 'compressibility', 'clip', 'imagereward', or 'imagenet' (required)
...
def main():
    ...
    parser.add_argument('--backend', type=str, choices=['edm', 'sd', 'sd15'], required=True, help='Backend: edm, sd, or sd15')
    parser.add_argument('--scorer', type=str, choices=['brightness', 'compressibility', 'clip', 'imagereward', 'imagenet'], required=True, help='Scorer name')
    ...
    # Validation
    # -----------
    if args.backend in {'sd', 'sd15'} and args.scorer == 'imagenet':
        raise ValueError('imagenet scorer is only available for edm backend')
    if args.backend == 'edm' and args.scorer in {'clip', 'imagereward'}:
        raise ValueError(f"{args.scorer} scorer is only available for sd backend")

    # -----------
    # SD Backend
    # -----------
    if args.backend in {'sd', 'sd15'}:
        ...
        scorer = get_scorer(
            ...
            ImageRewardScorer=ImageRewardScorer,
        )
        ...
        method = args.method
        num_inference_steps = 64 if args.backend == 'sd15' else 50
        guidance_scale = 7.5
        image_size = 512
        MASTER_PARAMS = {
            'N': args.N,
