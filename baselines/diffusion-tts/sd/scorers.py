...
class ImageRewardScorer(Scorer):
    ...
    def _load_model(self):
        ...
        fk_text_to_image = repo_root / "Fk-Diffusion-Steering" / "text_to_image"
        if str(fk_text_to_image) not in sys.path:
            sys.path.insert(0, str(fk_text_to_image))
        try:
            from fkd_diffusers.image_reward_utils import rm_load  # type: ignore

            self._model = rm_load("ImageReward-v1.0")
        except Exception:
    ...
    def _tensor_to_pil(img: torch.Tensor) -> Image.Image:
        ...
        if t.dim() == 4:
            t = t[0]
        if t.dim() != 3:
            raise ValueError(f"Expected 3D CHW tensor for image conversion, got shape={tuple(t.shape)}")
        ...
        if tf.min().item() < 0.0:
            tf = (tf + 1.0) / 2.0
        tf = tf.clamp(0.0, 1.0)
        arr = (tf * 255.0).round().to(torch.uint8).permute(1, 2, 0).numpy()
        return Image.fromarray(arr)

    def _images_to_pil_list(self, images):
        if isinstance(images, torch.Tensor):
            if images.dim() == 4:
                return [self._tensor_to_pil(x) for x in images]
            return [self._tensor_to_pil(images)]
        pil_images = []
        for img in images:
            if isinstance(img, torch.Tensor):
                if img.dim() == 4 and img.shape[0] > 1:
                    pil_images.extend([self._tensor_to_pil(x) for x in img])
                else:
    ...
    def __call__(self, images, prompts, timesteps=None):
        ...
        pil_images = self._images_to_pil_list(images)
        if isinstance(prompts, str):
            prompts = [prompts]
        if len(prompts) == 1 and len(pil_images) > 1:
            prompts = prompts * len(pil_images)
        scores = model.score_batched(prompts, pil_images)
        return torch.tensor([float(s) for s in scores], dtype=self.dtype)