...
from rbf.prior.flux import FluxPrior, FlowMatchEulerDiscreteScheduler
...
class FluxFillPrior(FluxPrior):
    ...
    def __init__(self, cfg):
        ...
        print("Loaded FluxFillPrior") if not sm.OFF_LOG else None


    def prepare_mask(self):
        image = Image.open(self.cfg.image_path).convert("RGB")
        mask_image = Image.open(self.cfg.mask_path).convert("L")

        image = self.pipeline.image_processor.preprocess(
            image, height=self.cfg.height, width=self.cfg.width
        )
        mask_image = self.pipeline.mask_processor.preprocess(
            mask_image, height=self.cfg.height, width=self.cfg.width
        )
        ...
        mask, masked_image_latents = self.pipeline.prepare_mask_latents(
            mask_image,
            masked_image,
            1, # NOTE: batch_size set to 1. This will be scaled dynamically depends on batch/particle size
            num_channels_latents,
            1,
            height,
            width,
        ...
        print("Prepare mask image latents", masked_image_latents.shape) if not sm.OFF_LOG else None
    ...
    def predict(
        ...
        _masked_image_latents = self.masked_image_latents.clone()
        ...
        for _i in range(0, len(x_t), self.cfg.minibatch_size):
            ...
            for k, v in self.cond.items():
                if k in ["encoder_hidden_states", "pooled_projections"]:
                    cur_cond[k] = v[_i:_i+cur_batch_size]
                else:
                    cur_cond[k] = v

                # print(k, cur_cond[k].shape)

            # print("cur_t", cur_t.shape)
            # print("cur_x_t_batch", cur_x_t_batch.shape)
            # print("self.pipeline.latent_image_ids", self.pipeline.latent_image_ids.shape)
            ...
            # NOTE: mask_image_latenet is scaled dynamically. 
            # For different applications have different sizes: (batch_size, n_particles, batch_size * n_particles)
            cur_masked_image_latents = _masked_image_latents.repeat(
                cur_x_t_batch.shape[0], *((1,) * (_masked_image_latents.dim() - 1))
            )

            cur_noise_pred = self.pipeline.transformer(
                hidden_states=torch.cat((cur_x_t_batch, cur_masked_image_latents), dim=2),
