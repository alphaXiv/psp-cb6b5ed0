        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # FK Steering Change
                if fkd_args is not None and fkd_args.get("use_smc") and fkd_args.get("log_reward_every_step"):
                    population_images = fkd.latent_to_decode_fn(x0_preds)
                    rs_candidates = fkd.reward_fn(population_images)
                    fkd.log_step(sampling_idx=i, rs_candidates=rs_candidates)
        ...
        # Match SDXL behavior: expose FK trajectory logs when SMC is enabled.
        if fkd_args is not None and fkd_args.get("use_smc") and "fkd" in locals():
            output.trajectory_log = fkd.trajectory_log
            output.trajectory_log_raw = fkd.trajectory_log_raw
        return output
