...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate tuning comparison figure (v2) as PNG and PDF."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def make_figure(*, profile_x_jitter_max: float, profile_y_jitter_max: float) -> plt.Figure:
    ...
    # v2: three rows on the left. HPS is now measured under IR guidance
    # (mean HPS of the IR-selected winner on GenEval prompts).
    metrics = ["IR", "HPS", "GenEval"]

    # PSP default schedule (k_init=8, ts=[16,32]/[8,16], ks=[4,2]); 3-seed mean on GenEval prompts.
    # All values are under IR guidance.
    fixed_mt = {
        "IR": [0.8266, 1.2238, 1.3798],
        "GenEval": [0.5737, 0.6449, 0.7466],
        "HPS": [0.2785, 0.2936, 0.3057],
    }
    # Best schedule found by grid search under matched compute (searched on IR Benchmark,
    # reported on GenEval prompts), also under IR guidance.
    fixed_tuned = {
        "IR": [0.8361, 1.2046, 1.3801],
        "GenEval": [0.5710, 0.6405, 0.7525],
        "HPS": [0.2783, 0.2954, 0.3056],
    }
