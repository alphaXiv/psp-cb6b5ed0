...
CUDA_VISIBLE_DEVICES=0 python launch_eval_runs.py \
  --use_smc \
  --model_idx 6 \
  --output_name "outputs/fk_sdv15_k4_t64_eta1_ir_benchmark_ir" \
...
CUDA_VISIBLE_DEVICES=0 python launch_eval_bestofn_runs.py \
  --model_idx 6 \
  --best_of_n 4 \
  --output_name "outputs/bestofn_sdv15_n4_t64_eta1_geneval" \
  --prompt_path "prompt_files/geneval_metadata.jsonl" \
  --metrics_to_compute "ImageReward#HumanPreference" \
  --guidance_reward_fn "ImageReward" \
  --num_inference_steps 64 \
...
CUDA_VISIBLE_DEVICES=0 python launch_eval_bestofn_runs.py \
  --model_idx 6 \
  --best_of_n 4 \
  --output_name "outputs/bestofn_sdv15_n4_t64_eta1_benchmark_ir" \
  --prompt_path "prompt_files/benchmark_ir.json" \
  --metrics_to_compute "ImageReward#HumanPreference" \
  --guidance_reward_fn "ImageReward" \
  --num_inference_steps 64 \
